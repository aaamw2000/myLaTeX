\documentclass[10pt]{article}
\usepackage{graphicx} % Required for inserting images
\usepackage[margin=2cm]{geometry}
\usepackage[inline]{enumitem}
\usepackage{hyperref}

\title{Amr's Resume}
\author{Amr Wahdan}
\date{}

\newcommand{\fancy}[1]{\Large\textbf{\textit{#1}}}

\graphicspath{{./resources/}}

\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    filecolor=magenta,      
    urlcolor=cyan,
    pdftitle={amr-wahdan-cv},
    pdfpagemode=FullScreen,
    }

\begin{document}
\pagenumbering{gobble}

%%% Header info %%%

\begin{center}
    \Large\fancy{\huge Amr Wahdan} \\
    \vspace{1em}
    CS student with a soild math/stat background and an exceptional sales pitch. Business Development specialist, Virtual assistant, ACQ. manager. \\

    \vspace{1em}
    (+20) 106-235-6288 \textbar\ \ aaamwahdan@gmail.com \\ % chktex 8
    \vspace{10px}
	\href{https://github.com/aaamw2000}{\includegraphics[width=0.04\textwidth]{github.png}} \href{https://www.upwork.com/freelancers/~010df5511c059eb9ea}{\includegraphics[width=0.04\textwidth]{upwork-in.png}} \href{https://www.linkedin.com/amr-a-wahdan}{\includegraphics[width=0.04\textwidth]{linkedin.png}} 
\end{center}


%%% Tech Skills %%%

\section*{Tech Skills}
\hrule
\noindent
\vspace{0.7em}
\Large

Experienced Business Development Professional with a background in client acquisition \& lead generation. Successfully managed outreach and on-boarding processes, achieving consistent results. Recognized as a top achiever across multiple roles, exploring new business opportunities and revitalizing campaigns.

\subsection*{\Large{Computer languages}}
\begin{itemize}
	\item R, PowerBI, LookerStudio -- Analyzing data, running statistical tests, and visualizing results.
	\item Python -- Basic scripting \& automation.
	\item Bash -- Maintaining servers \& daily use on my OpenSUSE Linux machine.
	\item \LaTeX -- capable of making neat \& fancy-looking reports; advanced skills from extensive use due to math background.
	\item Basic web development -- capable of maintain websites.
\end{itemize}

\subsection*{\Large{Human languages}}
\begin{itemize}
	\item Arabic -- Native speaker
	\item English -- Native-like speaker (112/120 \href{https://drive.google.com/file/d/1S9nyRJj5tY12hFw3_XthDpG-YLtbXHaj/view?usp=sharing}{TOEFL score})
\end{itemize}



%%% Education %%%

\section*{Education}
\hrule
\noindent

\begin{minipage}{0.3\textwidth}
	\vspace{1em}
	\begin{center}
		\includegraphics[width=\textwidth]{FOS_logo.jpg}
\end{center}
\end{minipage}
% \hspace{-2pt}
\begin{minipage}{0.7\textwidth}
\vspace{2em}
\begin{center}
    \begin{itemize}
        \item \large \fancy{Bachelor of Science (June 2025)}
    \end{itemize}
    \begin{itemize}
        \item \large \fancy{Major in Computer Science \& minor in Statistics}
    \end{itemize}
\end{center}
\end{minipage}

%%%% XP section %%%

\section*{Work Experience}
\hrule
\vspace{3em}

%%% Nedialo job %%%
\begin{minipage}{0.75\textwidth}
 \fancy{BUSINESS DEVELOPMENT MANAGER} \\
	{\large \href{https://www.linkedin.com/company/-nedialo/}{Nedialo} --- Virtual assistance company} \\
\begin{itemize}
    \item Reaching out to potential clients and securing new deals.
    \item On-boarding clients and following up with current ones.
    \item Driving growth and expanding market presence.
    \item Monthly average of 16-20 appointments booked; 2-3 on-boardings
\end{itemize}   
\end{minipage}
% \hspace{-5pt}
\hspace{10pt}
\begin{minipage}{0.2\textwidth}
\begin{center}
\vspace{20pt}
\includegraphics[width=\textwidth]{nedialo-black.jpg} \\
    \large\textbf{November 2024 \\October 2023} %%
\end{center}
\end{minipage} %%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{3em}

\noindent
%%% 2nd TT job %%%
\begin{minipage}{0.7\textwidth}
 \fancy{COLD CALLER} \\
	{\large \href{https://www.linkedin.com/company/tele-target}{Teletarget} --- Tele-marketing company} \\
\begin{itemize}
	\item Calling realtors (B2B) and wholesalers about off-market properties \& assignment contracts.
    \item Retained my position as a top achiever (top 3).
    \item Helped create new camps and revived dead ones.
    \item Helped stabilize management amid company fracture.
\end{itemize}   
\end{minipage}
\hspace{35pt}
\begin{minipage}{0.2\textwidth}
\begin{center}
\vspace{10pt}
\includegraphics[width=\textwidth]{tt_coldcalling.jpg} \\
    \large\textbf{September 2023\\June 2023} %%
\end{center}
\end{minipage} %%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{3em}

\noindent
%%% 1st TT job %%%
\begin{minipage}{0.7\textwidth}
 \fancy{COLD CALLER} \\
	{\large \href{https://www.linkedin.com/company/tele-target}{Teletarget} --- Tele-marketing company} \\
\begin{itemize}
    \item Calling home owners and gauging selling interest within camp criteria.
    \item Was a top achiever the entire time there.
    \item Had a daily average of around 6 leads per shift.
    \item Was offered a promotion to the QC department.
\end{itemize}   
\end{minipage}
\hspace{35pt}
\begin{minipage}{0.2\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{tt_coldcalling.jpg} \\
    \large\textbf{October 2022\\June 2022}
\end{center}
\end{minipage}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{3em}

%%% PBS job %%%

\noindent
\vspace{2em}
\begin{minipage}{0.7\textwidth}
 \fancy{COLD CALLER} \\
	{\large \href{https://www.linkedin.com/company/proper-business-solutions}{Proper Business solutions} --- Tele-marketing company} \\
\begin{itemize}
    \item Calling home owners and qualifying leads.
    \item One of select few who got 5+ leads per shift.
    \item Broke the spell of dead campaigns.
    \item Was known as the most helpful colleague there.
\end{itemize}   
\end{minipage}
\hspace{30pt}
\begin{minipage}{0.2\textwidth}
\begin{center}
\vspace{15pt}
\includegraphics[width=\textwidth]{pbs_lowquality_coldcalling.jpg} \\
    \large\textbf{June 2022\\March 2022}
\end{center}
\end{minipage}

% \vspace{1em}

%%%%%%%%%%%%%%%%
\section*{Volunteering and Leadership Roles}
\hrule
\vspace{1em}
\begin{itemize}
	\item \fancy{\href{https://www.linkedin.com/company/syndicate-of-scientific-professions-alexandria-نقابة-المهن-العلمية-الإسكندرية/}{Egyptian Syndicate of Scientific Professions}}: vice head of the public relations committee; responsible for representing the syndicate, brokering new deals, and revitalizing existing ones.
	\item \fancy{\href{https://www.linkedin.com/showcase/ecpc-egyptian-collegiate-programming-contest1/}{Egyptian Collegiate Programming Contest}}: community leader of the faculty of science competitive programming community; responsible for handling all official communications on behalf of the faculty, informing participants about the rules \& regulations, and organizing the trip to the contest.
	\item \fancy{\href{https://www.linkedin.com/company/bibalexofficial/}{Bibliotheca Alexandrina}}: head of the toddlers dept.; responsible for organizing science sessions, ensuring the safety and well-being of participants, and informing parents/guardians about available sessions and upcoming events.
	\item \fancy{\href{https://www.linkedin.com/company/egyptian-food-bank/}{Egyptian Food Bank}}: ERP specialist; responsible for tracking supplies, making deliveries, and informing management of needed shipment. I managed lots of excel sheets and helped the accountant there.
	\item \fancy{\href{https://www.linkedin.com/company/asafra-faith-church/}{Asafra Faith Church}}: organizational volunteer helping out with the yearly grand Christmas celebration; responsible for assisting guests, tracking supplies, and taking pictures of the event.
\end{itemize}

%%%%%%%%%%%%%%%%
\section*{Personal Qualities}
\hrule
\vspace{1em}
\begin{itemize*}
\item \fancy{Punctual \quad\quad}
\item \fancy{Professional \quad\quad}
\item \fancy{Team player \quad\quad}
\item \fancy{Tech savvy \quad\quad}
\item \fancy{Leader}
\end{itemize*}

\begin{center}
	{\tiny this document was produced using \LaTeX. View the source code here: \href{https://github.com/aaamw2000/myLaTeX/tree/main/pii/myCV}{github}}
\end{center}

\end{document}
